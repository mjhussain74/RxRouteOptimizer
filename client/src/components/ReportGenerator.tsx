...
export default function ReportGenerator({
  ...
  const [reportType, setReportType] = useState<
    "orders" | "routes" | "route-details" | "deliveries" | "prescriptions"
  >("orders");
  const [proofDialog, setProofDialog] = useState<{
    open: boolean;
    type: "signature" | "photo";
    data: string | null;
  }>({ open: false, type: "signature", data: null });
  ...
  const { data: ordersData = [] } = useQuery<OrderWithProof[]>({
    ...
    queryFn: async () => {
      ...
      if (selectedPharmacyId)
        params.append("pharmacyId", selectedPharmacyId.toString());
  ...
  const completedRoutes = filteredRoutes.filter(
    (r) => r.status === "complete" || r.status === "completed" || r.completedAt,
  );
  const activeRoutes = filteredRoutes.filter(
    (r) => r.status === "dispatched" || r.status === "active",
  );
  const totalDeliveries = filteredBatches.reduce(
    (acc: number, b: any) => acc + (b.totalDeliveries || 0),
    0,
  );
  ...
  const _generatePDFReport = async () => {
    ...
    if (reportType === "route-details" && routeReportData) {
      ...
      y += 6;
      doc.text(
        `Status: ${routeReportData.route.status.toUpperCase()}`,
        margin,
        y,
      );
      y += 6;
      if (routeReportData.route.completedAt) {
        doc.text(
          `Completed: ${new Date(routeReportData.route.completedAt).toLocaleString()}`,
          margin,
          y,
        );
        y += 6;
      ...
      const summaryItems = [
        {
          label: "Total Stops",
          value: String(routeReportData.summary.totalStops),
        },
        {
          label: "Completed",
          value: String(routeReportData.summary.completedStops),
        },
        {
          label: "Proof Collected",
          value: `${routeReportData.summary.stopsWithProof}/${routeReportData.summary.totalStops}`,
        },
        {
          label: "Rx Verified",
          value: `${routeReportData.summary.verifiedPrescriptions}/${routeReportData.summary.totalPrescriptions}`,
        },
      ];
      ...
      for (const stop of routeReportData.stops) {
        ...
        const rxText = stop.prescriptions
          .map((p) => `${p.rxNumber}${p.verified ? " ✓" : ""}`)
          .join("  ");
        const rxLines =
          stop.prescriptions.length > 0
            ? doc.splitTextToSize(rxText, contentW)
            : [];
        let estimatedH =
          9 +
          addrLines.length * 4.5 +
          12 +
          (rxLines.length > 0 ? rxLines.length * 4.5 + 6 : 0) +
          6;
        ...
        if (sigBase64 || photoBase64) {
          ...
          const imgAreaW = bothImages
            ? (contentW - 4) / 2
            : Math.min(contentW * 0.55, 95);
          ...
          if (photoBase64) {
            ...
            const photoY = bothImages
              ? y - (sigBase64 ? sigH + 4 + 3.5 : 0)
              : y;
            if (!bothImages) {
              doc.setFontSize(7);
              doc.setTextColor(100, 100, 100);
      ...
      doc.save(
        `proof-of-delivery-${routeReportData.route.name}-${now.toISOString().split("T")[0]}.pdf`,
      );
      return;
    }
    ...
    if (dateFrom || dateTo) {
      ...
      doc.setTextColor(100, 100, 100);
      doc.text(
        `Date Range: ${dateFrom || "Start"} — ${dateTo || "Present"}`,
        margin,
        y,
      );
      y += 8;
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Total Batches: ${filteredBatches.length}`, margin, y);
    y += 7;
    doc.text(`Total Deliveries: ${totalDeliveries}`, margin, y);
    y += 7;
    doc.text(`Completed Routes: ${completedRoutes.length}`, margin, y);
    y += 7;
    doc.text(`Active Routes: ${activeRoutes.length}`, margin, y);
    y += 7;
    ...
    if (reportType === "routes") {
      ...
      filteredRoutes.forEach((route: any, index: number) => {
        ...
        doc.text(
          `${index + 1}. ${route.name}  ·  ${driverName}  ·  ${status}`,
          margin,
          y,
        );
        y += 6;
      ...
      selectedBatchData.deliveries.forEach((delivery, index: number) => {
        ...
        doc.text(
          `${index + 1}. ${delivery.deliveryIdentifier || `DEL${delivery.id}`}  ·  ${delivery.addressText}`,
          margin,
          y,
        );
        y += 5;
        doc.text(
          `   ${delivery.customerName || "N/A"}  ·  ${delivery.status}  ·  ${rxCount} Rx`,
          margin,
          y,
        );
        y += 7;
      ...
      selectedBatchData.prescriptions.forEach((prescription, index: number) => {
        ...
        const delivery = selectedBatchData.deliveries.find(
          (d) => d.id === prescription.deliveryId,
        );
        doc.text(
    ...
    if (reportType === "routes") {
      ...
      const rows = filteredRoutes.map((route: any) => {
        return [
          ...
          route.dispatchedAt
            ? new Date(route.dispatchedAt).toLocaleDateString()
            : "",
          route.completedAt
      ...
      const rows = routeReportData.stops.map((stop) => {
        ...
        const verifiedCount = stop.prescriptions.filter(
          (p) => p.verified,
        ).length;
        return [
          stop.sequence,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400">
            Generate and export delivery reports with proof of delivery
          </p>
        </div>
        ...
        <CardContent className="p-0">
          ...
          >
            <TabsList className="w-full bg-slate-700/50 rounded-none border-b border-slate-700">
              <TabsTrigger
                value="orders"
                className="flex-1 data-[state=active]:bg-slate-600"
              >
                                ...
                                {order.proof.hasSignature && (
                                  <button
                                    onClick={() =>
                                      openProofDialog(
                                        "signature",
                                        order.proof?.signatureData || null,
                                      )
                                    }
            ...
            {isAdmin && (<TabsContent value="routes" className="mt-0">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-700/50">
                    <tr>
            ...
            {isAdmin && (<TabsContent value="route-details" className="mt-0">
              {!selectedRouteId ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div className="bg-slate-700/50 rounded-lg p-3">
                      ...
                      <p className="text-white font-medium">
                        {routeReportData.driver?.name || "Unassigned"}
                      </p>
                    </div>
                    <div className="bg-slate-700/50 rounded-lg p-3">
                      <p className="text-xs text-slate-400">Stops Completed</p>
                      <p className="text-white font-medium">
                        {routeReportData.summary.completedStops}/
                        {routeReportData.summary.totalStops}
                      </p>
                    </div>
                    <div className="bg-slate-700/50 rounded-lg p-3">
                      <p className="text-xs text-slate-400">Proof Collected</p>
                      <p className="text-white font-medium">
                        {routeReportData.summary.stopsWithProof}/
                        {routeReportData.summary.totalStops}
                      </p>
                    </div>
                    <div className="bg-slate-700/50 rounded-lg p-3">
                      ...
                      <p className="text-white font-medium">
                        {routeReportData.summary.verifiedPrescriptions}/
                        {routeReportData.summary.totalPrescriptions}
                      </p>
                    </div>
                  </div>
                          ...
                          <tr key={stop.id} className="hover:bg-slate-700/30">
                            ...
                            <td className="px-3 py-2">
                              {stop.prescriptions.length > 0 ? (
                                ...
                                >
                                  {
                                    stop.prescriptions.filter((p) => p.verified)
                                      .length
                                  }
                                  /{stop.prescriptions.length}
                                </span>
                              ) : (
                                <span className="text-xs text-slate-500">
                                  -
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-700">
                    {!selectedBatchId ? (
                      ...
                      selectedBatchData.prescriptions
                        .slice(0, 30)
                        .map((prescription) => {
                          const delivery = selectedBatchData.deliveries.find(
                            (d) => d.id === prescription.deliveryId,
      ...
      <Dialog
        ...
        onOpenChange={(open) => setProofDialog({ ...proofDialog, open })}
      >
        <DialogContent className="bg-slate-800 border-slate-700 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-white">
              {proofDialog.type === "signature"
                ? "Delivery Signature"
                : "Delivery Photo"}
            </DialogTitle>
          </DialogHeader>
