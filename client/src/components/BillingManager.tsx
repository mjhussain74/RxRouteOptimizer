...
export default function BillingManager({
  ...
  >({
    ...
    queryFn: async () => {
      let url: string;
      if (isPharmacyView) {
        url = "/api/billing/invoices/pharmacy";
      } else {
        url = filterPharmacyId
  ...
  const { data: pharmacies = [] } = useQuery<any[]>({
    ...
    enabled: !isPharmacyView, // ← don't fetch when pharmacy user
  });
  ...
  //     setGenerateAllMsg(
  //       `Generated ${data.generated} invoice(s), ${data.skipped} already existed or were skipped.`,
  //     );
  ...
  const totalDeliveries = invoices.reduce(
    (sum, inv) => sum + inv.totalDeliveries,
    0,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-white">Billing</h2>
          <p className="text-slate-400">
            View pharmacy invoices and configure delivery fees
          </p>
        </div>
              ...
              <div className="space-y-3">
                ...
                {activeTiers.length > 0 && (
                  <p className="text-slate-500 text-xs mt-2">
                    Deliveries beyond{" "}
                    {activeTiers[activeTiers.length - 1]?.maxMiles} miles are
                    not charged.
                  </p>
                )}
              </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <p className="text-slate-400 text-xs">
                          {invoice.totalDeliveries} deliveries
                        </p>
                      </div>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-slate-800/60">
                              ...
                              <th className="text-center px-4 py-2 text-slate-400 font-medium">
                                Status
                              </th>
                            </tr>
                          </thead>
                          <tbody>
                            {invoice.lineItems.map((item, idx) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-2 text-slate-300 text-xs text-right">
